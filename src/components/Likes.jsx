...
const LikePage = () => {
  ...
  const fetchUsers = async (currentUserId) => {
    ...
    const { data, error } = await supabase
      .from('user_profiles')
      .select('*')
      .neq('user_id', currentUserId);  // No queremos el perfil del usuario actual
  ...
  const handleLike = async (likedUserId) => {
    ...
    try {
      // Guardar el like en la tabla 'likes'
      ...
      // Si el like fue guardado correctamente, actualizamos el estado para reflejar el cambio
      setLikedUsers([...likedUsers, likedUserId]);

      // Verificar si hay un match (si el otro usuario también ha dado like)
  ...
  const checkForMatch = async (userId1, userId2) => {
    // Comprobar si el otro usuario ya ha dado like al usuario actual
    ...
    // Si se encuentra un like mutuo, se ha hecho un match
    if (data) {
      await supabase.from('matches').insert([
        { user1_id: userId1, user2_id: userId2, match_date: new Date().toISOString() },
      ]);
  ...
  return (
    <div style={styles.container}>
            ...
            <div key={user.user_id} style={styles.userCard}>
              ...
              <div style={styles.buttons}>
                <button
                  ...
                  onClick={() => {} /* aquí tu lógica de “no me gusta” */}
                >
                  No me gusta
                </button>
                <button
      ...
      {/* Botón con flecha para ir a Home */}
      <button
